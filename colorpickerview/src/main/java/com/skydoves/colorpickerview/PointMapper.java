...
class PointMapper {
    ...
    private static Point approximatedPoint(ColorPickerView colorPickerView, Point start, Point end) {
        if (colorPickerView.paletteDrawable != null) {
            start.x = MathUtils.clamp(start.x, 0, colorPickerView.getMeasuredWidth() - colorPickerView.selector.getMeasuredWidth() / 3);
            start.y = MathUtils.clamp(start.y, 0, colorPickerView.getMeasuredHeight());

            int color = colorPickerView.getColorFromBitmap(start.x, start.y);
            if (color == Color.TRANSPARENT) {
                ...
                return approximatedPoint(colorPickerView, start, end);
            } else {
                return start;
            }
        }
